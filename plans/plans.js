...
const app={
  ...
  showUserMenu(){
    ...
    setTimeout(()=>{
      document.addEventListener('click',function closeMenu(e){
        if(!menu.contains(e.target)&&!document.getElementById('userBtn').contains(e.target)){
          menu.remove();
  ...
  generateItinerary(gov,days,interests){
    ...
    interests.forEach(int=>{
      // محاولة الحصول على الأنشطة بناءً على اللغة المختارة
      let acts=[];
      if(gov.activities){
        // البحث عن الأنشطة باللغة الحالية
        const langSuffix=this.lang==='ar'?'':this.lang==='en'?'En':'Fr';
        const activityKey=int+langSuffix.charAt(0).toUpperCase()+langSuffix.slice(1);
        
        if(gov.activities[activityKey] && Array.isArray(gov.activities[activityKey])){
          acts=gov.activities[activityKey];
        } else if(gov.activities[int] && Array.isArray(gov.activities[int])){
          acts=gov.activities[int];
        }
      }
